...
class RockPaperScissors:
    ...
    async def wait_for_choice(self, ctx: commands.Context) -> str:
        def check(reaction: discord.Reaction, user: discord.Member) -> bool:
            return str(reaction.emoji) in self.OPTIONS and user == ctx.author and reaction.message == self.message

        reaction, _ = await ctx.bot.wait_for('reaction_add', check=check)
        return str(reaction.emoji)

    async def start(
        self, ctx: commands.Context, *, embed_color: Union[discord.Color, int] = 0x2F3136
    ) -> discord.Message:
        embed = discord.Embed(
